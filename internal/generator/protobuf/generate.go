...
func generateWithResult(ctx context.Context, root string, commands runner) (result GenerateResult, err error) {
	if root == "" {
		root = "."
	}
	...
	if !hasContracts {
		if err := reconcileEmptyGeneratedOutputs(ctx, root, module, serviceProject, commands); err != nil {
			return GenerateResult{}, err
		}
	...
	snapshot, err := snapshotGeneratedOutputs(root, serviceProject)
	if err != nil {
		return GenerateResult{}, err
	}
	committed := false
	defer func() {
		if !committed {
			if restoreErr := snapshot.restore(root, serviceProject); restoreErr != nil {
				result = GenerateResult{}
				err = errors.Join(err, fmt.Errorf("protobuf: rollback generated outputs: %w", restoreErr))
			}
		}
	}()
	...
	pruned, err := pruneStaleGeneratedProtobuf(root)
	if err != nil {
		return GenerateResult{}, err
	}
	if !serviceProject {
	...
	if pruned {
		if _, err := commands.Run(ctx, root, "go", "build", "./..."); err != nil {
			return GenerateResult{}, fmt.Errorf("protobuf: verify contract removal: %w; remove or update user-owned implementations of deleted RPCs", err)
		}
	}
	committed = true
...
const generatedProtobufHeader = "// Code generated by protoc-gen-go"

type outputSnapshot struct {
	protobuf        map[string][]byte
	transport       []byte
	transportExists bool
}

func snapshotGeneratedOutputs(root string, serviceProject bool) (outputSnapshot, error) {
	snapshot := outputSnapshot{protobuf: make(map[string][]byte)}
	...
	err := filepath.WalkDir(generatedRoot, func(path string, entry os.DirEntry, walkErr error) error {
		...
		if !bytes.HasPrefix(contents, []byte(generatedProtobufHeader)) {
			return nil
		}
		...
		snapshot.protobuf[relative] = contents
		return nil
	})
	if err != nil {
		return outputSnapshot{}, fmt.Errorf("protobuf: snapshot generated files: %w", err)
	}
	if serviceProject {
		path := filepath.Join(root, "internal", "transport", "grpc", "register.gen.go")
		contents, err := os.ReadFile(path)
		if err == nil {
			snapshot.transport, snapshot.transportExists = contents, true
...
func (snapshot outputSnapshot) restore(root string, serviceProject bool) error {
	generatedRoot := filepath.Join(root, "gen", "pb")
	if _, err := removeManagedProtobufFiles(generatedRoot, nil); err != nil {
		return err
	}
	for relative, contents := range snapshot.protobuf {
		path := filepath.Join(generatedRoot, relative)
	...
	if serviceProject {
		path := filepath.Join(root, "internal", "transport", "grpc", "register.gen.go")
		if snapshot.transportExists {
			return atomicWrite(path, snapshot.transport)
		}
...
func reconcileEmptyGeneratedOutputs(ctx context.Context, root, module string, serviceProject bool, commands runner) error {
	snapshot, err := snapshotGeneratedOutputs(root, serviceProject)
	if err != nil {
		return err
	}
	removed, err := removeManagedProtobufFiles(filepath.Join(root, "gen", "pb"), nil)
	if err != nil {
		return errors.Join(err, snapshot.restore(root, serviceProject))
	}
	if serviceProject {
		if err := writeTransport(root, module, nil); err != nil {
			return errors.Join(err, snapshot.restore(root, serviceProject))
		}
		if removed {
			if _, err := commands.Run(ctx, root, "go", "build", "./..."); err != nil {
				return errors.Join(
					fmt.Errorf("protobuf: verify contract removal: %w; remove or update user-owned implementations of deleted RPCs", err),
					snapshot.restore(root, serviceProject),
				)
			}
		}
	}
...
func pruneStaleGeneratedProtobuf(root string) (bool, error) {
	files, err := candidateFiles(root, "")
	if err != nil {
		return false, err
	}
	protoDirectory := filepath.Join(root, filepath.FromSlash(protoRoot))
	expected := make(map[string]bool, len(files)*2)
	for _, path := range files {
		...
		expected[base+".pb.go"] = true
		expected[base+"_grpc.pb.go"] = true
	}
	return removeManagedProtobufFiles(filepath.Join(root, "gen", "pb"), expected)
}

func removeManagedProtobufFiles(root string, keep map[string]bool) (bool, error) {
	removed := false
