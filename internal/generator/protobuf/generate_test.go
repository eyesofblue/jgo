...
func TestGenerateRunsBufAndCreatesOnlyMissingServiceStubs(t *testing.T) {
	...
	runner := &fakeRunner{}
	if err := generate(context.Background(), root, runner); err != nil {
	...
	for _, fragment := range []string{"type userServiceServer struct", "application.UserServiceGetUser(ctx, request)", "RegisterUserServiceServer"} {
	...
	if err := generate(context.Background(), root, &fakeRunner{}); err != nil {
