...
2. 执行 `make ci`。
3. 执行 `./scripts/verify-generation.sh`；除生成与构建外，必须通过 proto module → gRPC server → Web caller 的真实运行验收，包括 trace_id、3 秒超时、`Unavailable`、Management `/healthz` 与 readiness。
